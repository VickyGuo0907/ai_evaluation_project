...
class ExampleRAG:
    ...
    def __init__(
        ...
        Args:
            llm_client: LLM client with a generate() method (BaseLLMClient instance)
...
def default_rag_client(
    llm_client, logdir: str = "logs", model_name: str = "default"
) -> ExampleRAG:
    """
    Create a default RAG client with LLM client and optional retriever.
...
if __name__ == "__main__":
    from llm_client import OpenAIClient, OllamaClient
    ...
    if not api_key:
        print("No OPENAI_API_KEY found, using Ollama instead...")
        use_ollama = True
    ...
    if use_ollama:
        ...
        llm_client = OpenAIClient(client=OpenAI(api_key=api_key), model="gpt-4o")
        model_name = "gpt-4o"

    # Initialize RAG system with tracing enabled
    r = SimpleKeywordRetriever()
    rag_client = ExampleRAG(
        llm_client=llm_client, retriever=r, logdir="logs", model_name=model_name
    )
